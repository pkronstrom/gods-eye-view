...
test('no unchanged Realtime tool definition drifts silently', () => {
  ...
  // `bing-road` stack leaving `set_map_stack`'s enum, and — homelab fork — the
  // s2cloudless stack plus the FMI lightning layer joining their enums are the
  // known schema changes. Everything else must be byte-identical: an unnoticed edit
